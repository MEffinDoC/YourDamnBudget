...
/**
 * computeWeekPay supports:
 * - Simple OT: hours.regular, hours.ot, hours.otMultiplier
 * - Advanced OT: hours.ot15, hours.ot2 (when hours.mode==='advanced')
 */
...
/**
 * project(state) -> 12-week projection (basic)
 * income = weekly net from computeWeekPay
 */
export function project(state) {
  ...
  const mustPays = (state?.bills||[]).reduce((s,b)=>s + (Number(b.amount||0)/4.345), 0) // rough weeklyization
