// YDB v14b — restores all views + OT dropdown + “Apply” copy
...
let state=JSON.parse(localStorage.getItem(storeKey()))||{}; state.ui=state.ui||{onboarded:false,lang:'en',tone:'spicy'}; state.user=state.user||{paydayWeekday:5,faFundsPerWeek:50}; save();
const DOW=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'], money=n=>(n<0?'-':'')+'$'+Math.abs(+n||0).toFixed(2);
...
function renderHome(){
  ...
  const variables=(state.envelopes||[]).reduce((s,e)=>s+(+e.weeklyTarget||0),0), faf=+u.faFundsPerWeek||0, bankBal=+bank.currentBalance||0;
...
/* PLANNER (placeholder) */
function renderPlanner(){ app.appendChild(section('Crystal Ball (12-week)','<div class="help">Projection coming soon.</div>')); }

/* TIMESHEET */
function renderTimesheet(){
  const r=state.payRules||{baseHourly:20,withholdingRatio:.2}; state.hours=state.hours||{mode:'simple',regular:40,ot:0,otMultiplier:1.5}; save();
...
/* simple placeholders for other tabs so nothing is “blank” */
...
/* Router */
function render(){
  app.innerHTML=''; const view=document.querySelector('nav .tab.active')?.dataset.view||'home';
  if(view==='home')renderHome(); else if(view==='planner')renderPlanner(); else if(view==='timesheet')renderTimesheet();
  else if(view==='bills')renderBills(); else if(view==='events')renderEvents(); else if(view==='envelopes')renderEnvelopes();
  else if(view==='loans')renderLoans(); else if(view==='donate')renderDonate(); else if(view==='settings')renderSettings();
}
document.querySelectorAll('nav .tab').forEach(b=>b.onclick=()=>{document.querySelectorAll('nav .tab').forEach(x=>x.classList.remove('active'));b.classList.add('active');render();});
render();

/* First-run wizard */
if(!state.ui.onboarded && wizard){ import('./wizard.js?v=14b').then(m=>m.showWizard(state,save,render)); }