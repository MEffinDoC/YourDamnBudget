...
function renderPlanner(){ /* same as v12 */ let weeks=[]; try{ weeks=project(state)||[]; }catch{}; const wrap=section('Crystal Ball — 12-week', `
  ...
  function renderStep(){
    ...
    if(steps[step]==='bank'){
      ...
      inner += `
        ...
        <div class="step-hint">Add at least one bill — you can add more later.</div>
        <div class="grid cols-3">
        ...
        <div class="actions"><button id="back" class="button ghost">Back</button><span class="spacer"></span><button id="addOrNext" class="button primary">Add bill</button></div>
      `;
    }
    else if(steps[step]==='loan'){
      inner += `
        <h2>Your Damn Debts</h2>
        <div class="step-hint">Add a loan/IOU (optional now, easy later too).</div>
        <div class="grid cols-3">
        ...
        <div class="actions"><button id="back" class="button ghost">Back</button><span class="spacer"></span><button id="addOrNext" class="button primary">Add (or Skip)</button></div>
      `;
    }
    else {
      inner += `
    ...
    if(steps[step]==='bank'){
      $('#next').onclick = ()=>{
        const bal = Number($('#w_bank').value||0);
        const wd  = Number($('#w_weekday').value||5);
        state.bank.currentBalance = bal;
        (state.user = state.user || {}).paydayWeekday = wd;
      ...
      const add = ()=>{
        ...
        state.bills.push({ id: Date.now(), name, amount, dueDay: day });
        save();
        // After adding one, turn button into Next
        $('#addOrNext').textContent = 'Next';
        $('#addOrNext').onclick = ()=>{ step++; renderStep(); };
      };
      $('#addOrNext').onclick = add;
      $('#back').onclick = ()=>{ step--; renderStep(); };
    }
    else if(steps[step]==='loan'){
      const addOrSkip = ()=>{
        ...
        if(name && min && (day>=1 && day<=31)){
          state.loans.push({ id: Date.now(), name, minimumPayment:min, dueDay:day, balance:bal });
          save();
        }
        step++; renderStep();
      };
      $('#addOrNext').onclick = addOrSkip;
      $('#back').onclick = ()=>{ step--; renderStep(); };
    }
    else {
      $('#finish').onclick = ()=>{
        ...
        // soft refresh of home to reflect new numbers
