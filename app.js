// YDB app.js v15.2 — fixes: tab crash, no placeholders, adds Monthly Snapshot on Home
...
function renderHome(){
  ...
  // Build upcoming due items within pay period (exclude already paid)
  ...
  (state.bills||[]).forEach(b=>{
    for(let m=0;m<2;m++){
      ...
      if(d>=start&&d<end){const it={kind:'bill',id:b.id,name:b.name,dateISO:iso(d),amount:+b.amount}; if(!paid.has(paidKey(it))) items.push(it);}
    }
  });
  (state.loans||[]).forEach(l=>{
    for(let m=0;m<2;m++){
      const d=new Date(start.getFullYear(),start.getMonth()+m,l.dueDay);
      if(d>=start&&d<end){const it={kind:'loan',id:l.id,name:l.name,dateISO:iso(d),amount:+l.minimumPayment}; if(!paid.has(paidKey(it))) items.push(it);}
    }
  });
  (state.events||[]).forEach(e=>{
    const d=new Date(e.date);
    if(d>=start&&d<end){const it={kind:'event',id:e.id,name:e.name||'One-time',dateISO:iso(d),amount:+e.amount}; if(!paid.has(paidKey(it))) items.push(it);}
  }); // <-- fixed stray extra paren that could crash tabs

  items.sort((a,b)=>a.dateISO.localeCompare(b.dateISO)||a.name.localeCompare(b.name));

  // Sum anything already marked paid within this period
...
function renderPlanner(){
  ...
  let d=new Date(); for(let i=0;i<12;i++){const start=iso(d), left=weeklyNet - must - vars - faf;
    rows.push(`<tr><td>Week ${i+1}</td><td>${start}</td><td>${money(weeklyNet)}</td><td>${money(must)}</td><td>${money(vars)}</td><td>${money(faf)}</td><td class="${left<0?'negative':'positive'}">${money(left)}</td></tr>`);
    ...
    if(s.querySelector('#mode').value==='advanced'){
      ...
      hrs.innerHTML=`<div class="grid cols-3">
        ...
        <div><label>OT Multiplier</label><select id="mult">
          ...
          <option value="2"${(+h.otMultiplier||1.5)==2?' selected':''}>2×</option>
        </select></div></div>`;
    }
  }
  function preview(){
...
function renderSettings(){
  const s=section('Settings',`
    <div class="grid cols-3">
      <div><label>Bank balance</label><input id="bal" type="number" step="0.01" value="${state.bank.currentBalance}"><div class="help">What’s in your account right now.</div></div>
      <div><label>Weekly FAF</label><input id="faf" type="number" step="0.01" value="${state.user.faFundsPerWeek}"><div class="help">“Fuck Around Funds” cap each week.</div></div>
      <div><label>Payday weekday</label><select id="day">${DOW.map((d,i)=>`<option value="${i}" ${i===(state.user.paydayWeekday??5)?'selected':''}>${d}</option>`).join('')}</select><div class="help">Which day do checks land?</div></div>
    </div>
