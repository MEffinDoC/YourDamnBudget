// YDB v15 (modal, add-many, skippable)
...
export function showWizard(state,save,onFinish){
  ...
  state.bills=state.bills||[]; state.loans=state.loans||[]; save();
  let step=0; const steps=['bank','pay','bill','loan','done']; const $=s=>el.querySelector(s);
  function ui(html){el.innerHTML=`<div class="panel"><div class="progress"><div style="width:${Math.round(step/(steps.length-1)*100)}%"></div></div>${html}</div>`;document.body.classList.add('noscroll');el.classList.remove('hidden');}
  function bank(){ui(`<h2>Start here — Bank Balance</h2>
    <div class="grid cols-2"><div><label>Current balance</label><input id="b" type="number" step="0.01" value="${state.bank.currentBalance}"></div>
    <div><label>Payday weekday</label><select id="w">${DOW.map((d,i)=>`<option value="${i}" ${i===(state.user.paydayWeekday??5)?'selected':''}>${d}</option>`).join('')}</select></div></div>
  ...
  function pay(){ui(`<h2>Your Paycheck</h2>
    <div class="grid cols-3"><div><label>Base hourly</label><input id="base" type="number" step="0.01" value="${state.payRules.baseHourly}"></div>
    <div><label>Withholding (0–1)</label><input id="tax" type="number" step="0.01" value="${state.payRules.withholdingRatio}"></div>
    <div><label>Fuck Around Funds (weekly)</label><input id="faf" type="number" step="0.01" value="${state.user.faFundsPerWeek}"></div></div>
    <div class="grid cols-3"><div><label>Example Gross</label><input id="g" type="number" step="0.01"></div><div><label>Example Net</label><input id="n" type="number" step="0.01"></div><div><button id="est" class="button">Estimate withholding</button></div></div>
  ...
  function bill(){const rows=state.bills.map(b=>`<tr data-id="${b.id}"><td>${b.name}</td><td>$${(+b.amount).toFixed(2)}</td><td>${b.dueDay}</td><td><button data-del="${b.id}" class="button">Remove</button></td></tr>`).join('');
    ...
    <div class="grid cols-3"><div><input id="bn" placeholder="Name (Rent, Electric)"></div><div><input id="ba" type="number" step="0.01" placeholder="Amount"></div><div><input id="bd" type="number" min="1" max="31" placeholder="Due day"></div></div>
    ...
    $('#back').onclick=()=>{step=1;render()}; $('#add').onclick=()=>{const n=$('#bn').value.trim(),a=+$('#ba').value||0,d=+$('#bd').value||0;if(!n||!a||d<1||d>31){alert('Name, amount, due day 1–31');return;}state.bills.push({id:Date.now()+Math.random(),name:n,amount:a,dueDay:d});save();bill();}; 
  ...
  function loan(){const rows=state.loans.map(l=>`<tr data-id="${l.id}"><td>${l.name}</td><td>$${(+l.minimumPayment).toFixed(2)}</td><td>${l.dueDay}</td><td>${l.balance?('$'+(+l.balance).toFixed(2)):'-'}</td><td><button data-del="${l.id}" class="button">Remove</button></td></tr>`).join('');
    ...
    <div class="grid cols-3"><div><input id="ln" placeholder="Name (Car, CC)"></div><div><input id="lm" type="number" step="0.01" placeholder="Minimum"></div><div><input id="ld" type="number" min="1" max="31" placeholder="Due day"></div><div><input id="lb" type="number" step="0.01" placeholder="Balance (opt)"></div></div>
    ...
    $('#back').onclick=()=>{step=2;render()}; $('#add').onclick=()=>{const n=$('#ln').value.trim(),m=+$('#lm').value||0,d=+$('#ld').value||0,b=+$('#lb').value||undefined;if(!n||!m||d<1||d>31){alert('Name, minimum, due day 1–31');return;}state.loans.push({id:Date.now()+Math.random(),name:n,minimumPayment:m,dueDay:d,balance:b});save();loan();}; 
